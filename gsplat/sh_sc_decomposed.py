...
class _SphericalHarmonicsSCDecomposed(torch.autograd.Function):
    """Spherical Harmonics"""
    ...
    def forward(
        ...
        colors = _make_lazy_cuda_func("compute_sh_sc_decomposed_fwd")(sh_degree, dirs, dc, coeffs, masks)
    ...
    def backward(ctx, v_colors: Tensor):
        ...
        v_dc, v_coeffs, v_dirs = _make_lazy_cuda_func("compute_sh_sc_decomposed_bwd")(
            num_bases,
            sh_degree,
            dirs,
            dc,
...
def spherical_harmonics_sc_decomposed(
    degrees_to_use: int,
    dirs: Tensor,  # [..., 3]
    dc: Tensor,  # [..., 1, 1]
    coeffs: Tensor,  # [..., K, 1]
    ...
    Args:
        ...
        dirs: Directions. [..., 3]
        dc: the Coefficients of the first degree. [..., 1, 1]
    ...
    assert dc.shape[-1] == 1, dc.shape
    assert coeffs.shape[-1] == 1, coeffs.shape
    if masks is not None:
        assert masks.shape == dirs.shape[:-1], masks.shape
        masks = masks.contiguous()
    return _SphericalHarmonicsSCDecomposed.apply(

...
template <typename S>
__global__ void rasterize_to_vis_aware_weights(
    const uint32_t C,
    const uint32_t N,
    ...
    S pixel_weight = 0;
    if (inside) {
        pixel_weight = pixel_weights[pix_id];
    }
...
call_vis_aware_kernel(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_vis_aware_weights<float>,
    ...
    rasterize_to_vis_aware_weights<float><<<blocks, threads, shared_mem, stream>>>(
        C,
        N,
        n_isects,
        packed,

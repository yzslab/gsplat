...
template <typename T>
__device__ inline float evaluate_opacity_factor(const T dx, const T dy, const T *co)
{
    return 0.5f * (co[0] * dx * dx + co[2] * dy * dy) + co[1] * dx * dy;
}

template<typename T>
__device__ inline T max_contrib_power_rect_gaussian(
    const T *co,
    const T *mean,
    const vec2<T> rect_min,
    const vec2<T> rect_max,
    vec2<T>& max_pos,
    const uint32_t tile_size)
{

    const T patch_size = static_cast<T>(tile_size - 1);
    const T patch_size_squared = patch_size * patch_size;
    ...
    // not_in_y_range OR not_in_x_range -> outside the tile
    if ((not_in_y_range + not_in_x_range) > 0.0f)
    {
        // the corner of the tile closest to the center of the 2D Gaussian
        const T px = x_left * rect_min.x + (1.0f - x_left) * rect_max.x;
        const T py = y_above * rect_min.y + (1.0f - y_above) * rect_max.y;

        const T dx = copysign(T(patch_size), x_min_diff);
        const T dy = copysign(T(patch_size), y_min_diff);

        const T diffx = mean[0] - px;
        const T diffy = mean[1] - py;

        const T rcp_dxdxcox = __frcp_rn(patch_size_squared * co[0]); // = 1.0 / (dx*dx*co[0])
        const T rcp_dydycoz = __frcp_rn(patch_size_squared * co[2]); // = 1.0 / (dy*dy*co[2])

        const T tx = not_in_y_range * __saturatef((dx * co[0] * diffx + dx * co[1] * diffy) * rcp_dxdxcox);
        const T ty = not_in_x_range * __saturatef((dy * co[1] * diffx + dy * co[2] * diffy) * rcp_dydycoz);
        max_pos = {px + tx * dx, py + ty * dy};

        const T max_pos_diff_x = mean[0] - max_pos.x;
        const T max_pos_diff_y = mean[1] - max_pos.y;
        max_contrib_power = evaluate_opacity_factor<T>(max_pos_diff_x, max_pos_diff_y, co);
    }
...
template <typename T>
__global__ void isect_tiles_tile_based_culling(
    ...
    const T *co = conics + 3 * idx;
    const T *xy = means2d + 2 * idx;
    constexpr float opacity_threshold = 1.0f / 255.0f;
    const T opacity_factor_threshold = static_cast<T>(logf(static_cast<float>(opacities[idx]) / opacity_threshold)); // opacity * 255.
    ...
    for (int32_t i = tile_min.y; i < tile_max.y; ++i) {
        for (int32_t j = tile_min.x; j < tile_max.x; ++j) {
            // the bounding box in pixel unit of the current tile
            const vec2<T> tile_bbox_min(static_cast<T>(j * tile_size), static_cast<T>(i * tile_size));
            const vec2<T> tile_bbox_max(static_cast<T>((j + 1) * tile_size - 1), static_cast<T>((i + 1) * tile_size - 1));

            vec2<T> max_pos;
            T max_opac_factor = static_cast<T>(0.f);
            max_opac_factor = max_contrib_power_rect_gaussian<T>(co, xy, tile_bbox_min, tile_bbox_max, max_pos, tile_size);
    ...
    auto offset_to = cum_tiles_per_gauss[idx];
    int64_t unused_key = INT32_MAX;
    unused_key <<= 32;
    const float depth = FLT_MAX;
    unused_key |= *((uint32_t*)&depth);
    for (; cur_idx < offset_to; ++cur_idx) {
        isect_ids[cur_idx] = unused_key;
        flatten_ids[cur_idx] = -1;
    }
}
...
__global__ void isect_offset_encode_tile_based_culling(
    ...
    int32_t *__restrict__ actual_n_isects
) {
    ...
    auto is_valid_key = isect_id_curr != INT32_MAX;
    ...
    uint32_t n_all_tiles = C * n_tiles;
    ...
    if (idx == 0) {
        if (is_valid_key) {
            ...
            *actual_n_isects = 0;
            for (uint32_t i = 0; i < n_all_tiles; ++i) {
                offsets[i] = static_cast<int32_t>(idx);
            }
        }
    }
    ...
    if (idx == n_isects - 1) {
        if (is_valid_key) {
            *actual_n_isects = static_cast<int32_t>(n_isects);
    ...
    if (idx > 0) {
        ...
        if (is_valid_key) {
            ...
            *actual_n_isects = static_cast<int32_t>(idx);
            for (uint32_t i = id_prev + 1; i < n_all_tiles; ++i)
                offsets[i] = static_cast<int32_t>(idx);
        }
    }
}
...
std::tuple<torch::Tensor, torch::Tensor> isect_offset_encode_tile_based_culling_tensor(
    ...
    torch::Tensor actual_n_isects = torch::zeros(
        {1,},
    ...
    if (n_isects) {
        ...
        isect_offset_encode_tile_based_culling<<<
            ...
            actual_n_isects.data_ptr<int32_t>()
        );
    } else {
        offsets.fill_(0);
    }
    return std::make_tuple(offsets, actual_n_isects);
}

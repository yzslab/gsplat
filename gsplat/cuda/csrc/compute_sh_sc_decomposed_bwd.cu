...
template <typename T>
__global__ void compute_sh_sc_decomposed_bwd_kernel(
    const uint32_t N,
    const uint32_t K,
    ...
    const T *__restrict__ v_colors,   // [N, 1]
    T *__restrict__ v_dc,             // [N, 1, 1]
    T *__restrict__ v_coeffs,         // [N, K, 1]
    ...
    sh_mc_coeffs_to_color_fast_decomposed_vjp<T, 1>(
        degrees_to_use,
        c,
        dirs[elem_id],
        dc + elem_id,
...
compute_sh_sc_decomposed_bwd_tensor(
    const uint32_t K,
    const uint32_t degrees_to_use,
    ...
    TORCH_CHECK(v_colors.size(-1) == 1, "v_colors must have last dimension 1");
    ...
    torch::Tensor v_dc = torch::zeros_like(dc);
    ...
    if (N) {
        compute_sh_sc_decomposed_bwd_kernel<float>
    ...
    return std::make_tuple(v_dc, v_coeffs, v_dirs); // [..., 1, 1], [..., K, 1], [..., 3]
}

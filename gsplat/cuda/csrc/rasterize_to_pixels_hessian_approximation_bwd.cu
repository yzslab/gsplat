...
__global__ void rasterize_to_pixels_hessian_approximation_bwd_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    // grad inputs
    S *__restrict__ v_G2, // [C, N] or [nnz]
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
            ...
            if (valid) {
                ...
                //     };
                //     v_xy_local = {
                //         v_sigma * (conic.x * delta.x + conic.y * delta.y),  // - opacity * vis * v_alpha * (conic.x * d.x + conic.y * d.y)
                //         // = opacity * v_alpha * (-vis * conic.x * d.x - vis * conic.y * d.y)
                //         // being lack a 0.5 * W
...
call_kernel_with_dim(
    ...
    torch::Tensor v_G2 = torch::zeros_like(opacities);
    ...
    if (n_isects) {
        ...
        if (cudaFuncSetAttribute(
                rasterize_to_pixels_hessian_approximation_bwd_kernel<CDIM, float>,
    ...
    return std::make_tuple(
        v_G2, v_means2d, v_conics, v_colors, v_opacities
    );
}
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
rasterize_to_pixels_hessian_approximation_bwd_tensor(
    // Gaussian parameters
    const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,                    // [C, N, 3] or [nnz, 3]

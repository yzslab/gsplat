...
template <typename T>
__global__ void compute_sh_decomposed_bwd_kernel(
    const uint32_t N,
    const uint32_t K,
    ...
    sh_coeffs_to_color_fast_decomposed_vjp(
        degrees_to_use,
        c,
        dirs[elem_id],
        dc + elem_id * 3,
...
compute_sh_decomposed_bwd_tensor(
    const uint32_t K,
    const uint32_t degrees_to_use,
    ...
    if (N) {
        compute_sh_decomposed_bwd_kernel<float>
    ...
    return std::make_tuple(v_dc, v_coeffs, v_dirs); // [..., 1, 3], [..., K, 3], [..., 3]
}

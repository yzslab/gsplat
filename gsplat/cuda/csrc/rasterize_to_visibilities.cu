...
template <typename S>
__global__ void rasterize_to_visibilities(
    const uint32_t C,
    const uint32_t N,
    ...
    int32_t *__restrict__ n_hit_pixels,       // [N]
    S *__restrict__ opacity_scores,           // [N]
    S *__restrict__ alpha_scores,             // [N]
    S *__restrict__ visibility_scores         // [N]
) {
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // Visibilities
            atomicAdd(&n_hit_pixels[g], 1);
            atomicAdd(&opacity_scores[g], opac);
            atomicAdd(&alpha_scores[g], alpha);
            atomicAdd(&visibility_scores[g], vis);
...
call_kernel(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_visibilities<float>,
    ...
    rasterize_to_visibilities<float><<<blocks, threads, shared_mem, stream>>>(
        C,
        N,
        n_isects,
        packed,
        ...
        n_hit_pixels.data_ptr<int32_t>(),
        opacity_scores.data_ptr<float>(),
        alpha_scores.data_ptr<float>(),
        visibility_scores.data_ptr<float>()
    );

    return std::make_tuple(
        n_hit_pixels, opacity_scores, alpha_scores, visibility_scores
    );
}

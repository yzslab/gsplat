#ifndef GSPLAT_SPHERICAL_MC_HARMONICS_CUH
#define GSPLAT_SPHERICAL_MC_HARMONICS_CUH
...
inline __device__ void sh_mc_coeffs_to_color_fast_decomposed(
    ...
    T result = 0.2820947917738781f * dc[c];
    if (degree >= 1) {
        ...
        result +=
            0.48860251190292f * (-y * coeffs[0 * C + c] +
                                 z * coeffs[1 * C + c] - x * coeffs[2 * C + c]);
        if (degree >= 2) {
            T z2 = z * z;
            ...
            result += pSH4 * coeffs[3 * C + c] + pSH5 * coeffs[4 * C + c] +
                      pSH6 * coeffs[5 * C + c] + pSH7 * coeffs[6 * C + c] +
                      pSH8 * coeffs[7 * C + c];
            if (degree >= 3) {
                ...
                result +=
                    pSH9 * coeffs[8 * C + c] + pSH10 * coeffs[9 * C + c] +
                    pSH11 * coeffs[10 * C + c] + pSH12 * coeffs[11 * C + c] +
                    pSH13 * coeffs[12 * C + c] + pSH14 * coeffs[13 * C + c] +
                    pSH15 * coeffs[14 * C + c];
...
template <typename T, uint32_t C>
inline __device__ void sh_mc_coeffs_to_color_fast_decomposed_vjp(
    const uint32_t degree, // degree of SH to be evaluated
    const uint32_t c,      // color channel
    const vec3<T> &dir,    // [3]
    const T *dc,           // [1]
    ...
    v_dc[c] = 0.2820947917738781f * v_colors_local;
    if (degree < 1) {
        return;
    }
    T inorm = rsqrtf(dir.x * dir.x + dir.y * dir.y + dir.z * dir.z);
    ...
    v_coeffs[0 * C + c] = -0.48860251190292f * y * v_colors_local;
    v_coeffs[1 * C + c] = 0.48860251190292f * z * v_colors_local;
    v_coeffs[2 * C + c] = -0.48860251190292f * x * v_colors_local;

    if (v_dir != nullptr) {
        v_x += -0.48860251190292f * coeffs[2 * C + c] * v_colors_local;
        v_y += -0.48860251190292f * coeffs[0 * C + c] * v_colors_local;
        v_z += 0.48860251190292f * coeffs[1 * C + c] * v_colors_local;
    }
    if (degree < 2) {
        if (v_dir != nullptr) {
    ...
    T pSH8 = 0.5462742152960395f * fC1;
    T pSH4 = 0.5462742152960395f * fS1;
    v_coeffs[3 * C + c] = pSH4 * v_colors_local;
    v_coeffs[4 * C + c] = pSH5 * v_colors_local;
    v_coeffs[5 * C + c] = pSH6 * v_colors_local;
    v_coeffs[6 * C + c] = pSH7 * v_colors_local;
    v_coeffs[7 * C + c] = pSH8 * v_colors_local;
    ...
    if (v_dir != nullptr) {
        ...
        v_x += v_colors_local *
               (pSH4_x * coeffs[3 * C + c] + pSH8_x * coeffs[7 * C + c] +
                pSH7_x * coeffs[6 * C + c]);
        v_y += v_colors_local *
               (pSH4_y * coeffs[3 * C + c] + pSH8_y * coeffs[7 * C + c] +
                pSH5_y * coeffs[4 * C + c]);
        v_z += v_colors_local *
               (pSH6_z * coeffs[5 * C + c] + pSH7_z * coeffs[6 * C + c] +
                pSH5_z * coeffs[4 * C + c]);
    }
    ...
    T pSH15 = -0.5900435899266435f * fC2;
    T pSH9 = -0.5900435899266435f * fS2;
    v_coeffs[8 * C + c] = pSH9 * v_colors_local;
    v_coeffs[9 * C + c] = pSH10 * v_colors_local;
    v_coeffs[10 * C + c] = pSH11 * v_colors_local;
    v_coeffs[11 * C + c] = pSH12 * v_colors_local;
    v_coeffs[12 * C + c] = pSH13 * v_colors_local;
    v_coeffs[13 * C + c] = pSH14 * v_colors_local;
    v_coeffs[14 * C + c] = pSH15 * v_colors_local;
    ...
    if (v_dir != nullptr) {
        ...
        v_x += v_colors_local *
               (pSH9_x * coeffs[8 * C + c] + pSH15_x * coeffs[14 * C + c] +
                pSH10_x * coeffs[9 * C + c] + pSH14_x * coeffs[13 * C + c] +
                pSH13_x * coeffs[12 * C + c]);

        v_y += v_colors_local *
               (pSH9_y * coeffs[8 * C + c] + pSH15_y * coeffs[14 * C + c] +
                pSH10_y * coeffs[9 * C + c] + pSH14_y * coeffs[13 * C + c] +
                pSH11_y * coeffs[10 * C + c]);

        v_z += v_colors_local *
               (pSH12_z * coeffs[11 * C + c] + pSH13_z * coeffs[12 * C + c] +
                pSH11_z * coeffs[10 * C + c] + pSH14_z * coeffs[13 * C + c] +
                pSH10_z * coeffs[9 * C + c]);
    }
    ...
    T pSH24 = 0.6258357354491763f * fC3;
    T pSH16 = 0.6258357354491763f * fS3;
    v_coeffs[15 * C + c] = pSH16 * v_colors_local;
    v_coeffs[16 * C + c] = pSH17 * v_colors_local;
    v_coeffs[17 * C + c] = pSH18 * v_colors_local;
    v_coeffs[18 * C + c] = pSH19 * v_colors_local;
    v_coeffs[19 * C + c] = pSH20 * v_colors_local;
    v_coeffs[20 * C + c] = pSH21 * v_colors_local;
    v_coeffs[21 * C + c] = pSH22 * v_colors_local;
    v_coeffs[22 * C + c] = pSH23 * v_colors_local;
    v_coeffs[23 * C + c] = pSH24 * v_colors_local;
    ...
    if (v_dir != nullptr) {
        ...
        v_x += v_colors_local *
               (pSH16_x * coeffs[15 * C + c] + pSH24_x * coeffs[23 * C + c] +
                pSH17_x * coeffs[16 * C + c] + pSH23_x * coeffs[22 * C + c] +
                pSH18_x * coeffs[17 * C + c] + pSH22_x * coeffs[21 * C + c] +
                pSH21_x * coeffs[20 * C + c]);
        v_y += v_colors_local *
               (pSH16_y * coeffs[15 * C + c] + pSH24_y * coeffs[23 * C + c] +
                pSH17_y * coeffs[16 * C + c] + pSH23_y * coeffs[22 * C + c] +
                pSH18_y * coeffs[17 * C + c] + pSH22_y * coeffs[21 * C + c] +
                pSH19_y * coeffs[18 * C + c]);
        v_z += v_colors_local *
               (pSH20_z * coeffs[19 * C + c] + pSH21_z * coeffs[20 * C + c] +
                pSH19_z * coeffs[18 * C + c] + pSH22_z * coeffs[21 * C + c] +
                pSH18_z * coeffs[17 * C + c] + pSH23_z * coeffs[22 * C + c] +
                pSH17_z * coeffs[16 * C + c]);
...
#endif // GSPLAT_SPHERICAL_MC_HARMONICS_CUH